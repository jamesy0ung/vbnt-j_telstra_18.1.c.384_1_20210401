...
GetConnectionTypeInfo(struct upnphttp * h, const char * action)
{
	static const char resp[] =
		...
		"</u:GetConnectionTypeInfoResponse>";
	char *wanipc=strstr(h->req_buf,"WANIPConnection:");
	char body[512];
	int bodylen;
	int ver = 1;
	...
	if (wanipc)
	{
	    char *version = wanipc + strlen("WANIPConnection:");
	    syslog(LOG_DEBUG, "version=%s",version);
	    ver=atoi(version);
	}

	bodylen = snprintf(body, sizeof(body), resp, ver);
...
GetCommonLinkProperties(struct upnphttp * h, const char * action)
{
	...
	if((downstream_bitrate == 0) || (upstream_bitrate == 0))
	{
		...
		{
			if(downstream_bitrate == 0) downstream_bitrate = data.downstream_bitrate;
			if(upstream_bitrate == 0) upstream_bitrate = data.upstream_bitrate;
			strcpy(wantype, data.wantype);
		}
	}
	...
	bodylen = snprintf(body, sizeof(body), resp,
	    action, "urn:schemas-upnp-org:service:WANCommonInterfaceConfig:1",
		wantype, upstream_bitrate, downstream_bitrate,
	    status, action);
	BuildSendAndCloseSoapResp(h, body, bodylen);
}
...
GetStatusInfo(struct upnphttp * h, const char * action)
{
    ...
    bodylen = snprintf(body, sizeof(body), resp,
        action, "urn:schemas-upnp-org:service:WANIPConnection:", ver,
...
void hostnameToIp(char *hostname , char **ip)
{
    ...
    for(i = 0; addr_list[i] != NULL; i++)
    {
        ...
        asprintf(ip,"%s", inet_ntoa(*addr_list[i]));
        break;
    }
...
AddPortMapping(struct upnphttp * h, const char * action)
{
    ...
    /* If r_host is empty we handle it as a wildcard */
    if (!r_host || strcmp(r_host,"") == 0)
        r_host = "*";

    /* Verify if r_host is a wildcard or a valid ip address or a valid hostname */
    if (strcmp(r_host, "*"))
    {
        if (! isValidIpAddress(r_host))
        {
            hostnameToIp(r_host, &r_host_ip);

            if (r_host_ip)
                r_host = r_host_ip;
            else
            {
                syslog(LOG_ERR, "Failed to convert remote hostname '%s' to ip address", r_host);
                ClearNameValueList(&data);
                SoapError(h, 402, "Invalid Args");
                return;
            }
	...
	/* if internal ip not valid assume hostname and convert */
	...
	syslog(LOG_INFO, "%s: ext port %hu to %s:%hu protocol %s for: %s leaseduration=%u rhost=%s extif=%s",
	       action, eport, int_ip, iport, protocol, desc, leaseduration,
	       r_host ? r_host : "NULL", ext_if_name);

	r = upnp_redirect(ext_if_name, r_host, eport, int_ip, iport, protocol, desc, leaseduration);
...
static void AddAnyPortMapping(struct upnphttp * h, const char * action)
{
    ...
    if (strcmp(r_host, "*"))
    {
        if (!isValidIpAddress(r_host))
        {
            ...
            {
                syslog(LOG_ERR,
                        "Failed to convert remote hostname '%s' to ip address",
                        r_host);
                ClearNameValueList(&data);
                SoapError(h, 402, "Invalid Args");
                return;
    ...
    if (inet_pton(AF_INET, int_ip, &result_ip) <= 0)
    {
        ...
        {
            syslog(LOG_ERR, "Failed to convert hostname '%s' to ip address",
                    int_ip);
            ClearNameValueList(&data);
            SoapError(h, 402, "Invalid Args");
            return;
    ...
    for (;;)
    {
        r = upnp_redirect(ext_if_name, r_host, eport, int_ip, iport, protocol,
                desc, leaseduration);
    ...
    case 0: /* success */
        bodylen = snprintf(body, sizeof(body), resp, action,
                SERVICE_TYPE_WANIPC, eport, action);
        BuildSendAndCloseSoapResp(h, body, bodylen);
        break;
    case -2: /* already redirected */
        SoapError(h, 718, "ConflictInMappingEntry");
...
GetSpecificPortMappingEntry(struct upnphttp * h, const char * action)
{
	 ...
	 * a priority. */
	r = upnp_get_redirection_infos(ext_if_name, eport, protocol, &iport,
	                               int_ip, sizeof(int_ip),
	                               desc, sizeof(desc),
	                               rhost, sizeof(rhost),
	                               &leaseduration);

	if ((r >= 0) && ((!strcmp(r_host, "*") && !strcmp(rhost, "")) || (!strcmp(rhost, r_host))))
	{
	        syslog(LOG_INFO, "%s: rhost='%s' extif=%s %s %s found => %s:%u desc='%s'",
	               action,
	               r_host ? r_host : "NULL", ext_if_name, ext_port, protocol, int_ip,
...
DeletePortMapping(struct upnphttp * h, const char * action)
{
	...
	r = upnp_delete_redirection(ext_if_name, eport, protocol);
...
DeletePortMappingRange(struct upnphttp * h, const char * action)
{
	...
	port_list = upnp_get_portmappings_in_range(ext_if_name, startport, endport,
	                                           protocol, &number);
	...
	for(i = 0; i < number; i++)
	{
		r = upnp_delete_redirection(ext_if_name, port_list[i], protocol);
...
GetGenericPortMappingEntry(struct upnphttp * h, const char * action)
{
	...
	rhost[0] = '\0';
	r = upnp_get_redirection_infos_by_index((int)index, ext_if_name, &eport, protocol, &iport,
                                            iaddr, sizeof(iaddr),
	                                        desc, sizeof(desc),
	                                        rhost, sizeof(rhost),
	                                        &leaseduration);
...
*/
	...
	for(i = 0; number > 0 && i < list_size; i++)
	{
		...
		rhost[0] = '\0';
		r = upnp_get_redirection_infos(ext_if_name, port_list[i], protocol, &iport,
		                               int_ip, sizeof(int_ip),
		                               desc, sizeof(desc),
		                               rhost, sizeof(rhost),
		                               &leaseduration);
...
SetDefaultConnectionService(struct upnphttp * h, const char * action)
{
	...
	if(p) {
...
#endif
		{
			...
			/*Replaced BuildSendAndCloseSoapResp(h, resp, sizeof(resp)-1); with SoapError(h, 501, "ActionFailed");
			to overcome UPnP Certification Issue */
//			BuildSendAndCloseSoapResp(h, resp, sizeof(resp)-1);
			SoapError(h, 501, "ActionFailed");
		}
	} else {
...
QueryStateVariable(struct upnphttp * h, const char * action)
{
	...
	else if(strcmp(var_name, "PortMappingNumberOfEntries") == 0)
	{
		...
		snprintf(strn, sizeof(strn), "%i",
		         upnp_get_portmapping_number_of_entries(ext_if_name));
		bodylen = snprintf(body, sizeof(body), resp,
                           action, "urn:schemas-upnp-org:control-1-0",
		                   strn, action);
		BuildSendAndCloseSoapResp(h, body, bodylen);
...
ExecuteSoapAction(struct upnphttp * h, const char * action, int n)
{
	...
	if (!strcmp(ext_if_name, ""))
	{
	    SoapError(h, 501, "Interface is not up/configured");
	    return;
	}
	...
	if(p)
	{
		...
		for (i = 0; soapMethods[i].methodName; i++)
        {
            ...
            if ((len == methodlen)
                    && memcmp(p, soapMethods[i].methodName, len) == 0)
            {
