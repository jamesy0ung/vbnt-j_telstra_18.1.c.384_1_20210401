...
static int
addnatrule(const char * ifname, int proto, unsigned short eport,
           const char * iaddr, unsigned short iport,
           const char * rhost);

static int
add_filter_rule(const char * ifname, int proto, const char * rhost,
                const char * iaddr, unsigned short iport);
...
static void
add_redirect_desc(const char * ifname, unsigned short eport, int proto,
...
static void
del_redirect_desc(const char * ifname, unsigned short eport, int proto)
{
	...
	while(p)
	{
		if(!strcmp(p->ifname, ifname) && p->eport == eport && p->proto == proto)
		{
			if(!last)
...
static void
get_redirect_desc(const char * ifname, unsigned short eport, int proto,
                  ...
                  unsigned int * timestamp)
{
	...
	{
		if((!strcmp(p->ifname, ifname) || !strcmp(ifname, "*")) && (p->eport == eport) && (p->proto == (short)proto))
		{
			if(desc)
...
{
    UNUSED(ifname);
    del_redirect_desc(ifname, eport, proto);
    add_redirect_desc(ifname, eport, proto, desc, timestamp);
    return 0;
}


void update_redirect_rule_timestamp(unsigned short eport, int proto, unsigned int timestamp)
{
    struct rdr_desc * p;
				   ...
				   const char * desc, unsigned int timestamp)
{
	...
	r = addnatrule(ifname, proto, eport, iaddr, iport, rhost);
	if(r >= 0)
		add_redirect_desc(ifname, eport, proto, desc, timestamp);
	return r;
}
                 ...
                 int proto, const char * desc)
{
	...
	return add_filter_rule(ifname, proto, rhost, iaddr, iport);
}
...
#endif
		{
		    if((proto==e->ip.proto) && (!strcmp(e->ip.iniface,ifname)))
			{
				...
				*iport = ntohs(mr->range[0].min.all);
				get_redirect_desc(ifname, eport, proto, desc, desclen, timestamp);
				if(packets)
...
get_redirect_rule_by_index(int index,
                           char * ifname, char *ifnamerule, int ifnamerulelen, unsigned short * eport,
                           char * iaddr, int iaddrlen, unsigned short * iport,
...
#endif
		{
			if (!strcmp(e->ip.iniface, ifname) || !strcmp(ifname, "*"))
			{
				if(i==index)
				{
					*proto = e->ip.proto;
					...
					*iport = ntohs(mr->range[0].min.all);
					...
					if (ifnamerule && ifnamerulelen > 0)
					{
					    snprintf(ifnamerule ,ifnamerulelen, "%s", e->ip.iniface);
					}
 ...
 */
int
delete_redirect_and_filter_rules(const char * ifname, unsigned short eport, int proto)
{
	int r = -1;
	unsigned index = 0;
	unsigned i = 0;
           ...
           const char * rhost)
{
	...
	e->ip.proto = proto;
	snprintf(e->ip.iniface, IFNAMSIZ, "%s", ifname);
...
unsigned short *
get_portmappings_in_range(const char * ifname, unsigned short startport, unsigned short endport,
