...
#ifdef ENABLE_LEASEFILE
static int
lease_file_add(const char * extif,
               const char *rhost,
               ...
               unsigned int timestamp)
{
	...
	fprintf(fd, "%s:%s:%hu:%s:%s:%hu:%u:%s\n",
	        ((proto==IPPROTO_TCP)?"TCP":"UDP"), extif, eport, rhost, iaddr, iport,
	        timestamp, desc);
	fclose(fd);
...
static int
lease_file_remove(const char * ifname, unsigned short eport, int proto)
{
	FILE* fd, *fdt;
	int tmp;
	char buf[512];
	...
	snprintf( str, sizeof(str), "%s:%s:%u", ((proto==IPPROTO_TCP)?"TCP":"UDP"), ifname ? ifname : "(null)", eport);
	str_size = strlen(str);
...
int reload_from_lease_file()
{
	...
	while(fgets(line, sizeof(line), fd)) {
		...
		*(p++) = '\0';
		...
		p = strchr(p, ':');
		if(!p) {
		    syslog(LOG_ERR, "extif unrecognized data in lease file");
		    continue;
		}
		*(p++)='\0';
        rhost = strchr(p, ':');
        if(!rhost) {
            printf("eport unrecognized data in lease file");
            continue;
        }
        *(rhost++) = '\0';
        ...
        p = strchr(rhost, ':');
        if(!p) {
            printf("rhost unrecognized data in lease file");
            continue;
        }
        *(p++) = '\0';
        iaddr = p;
        p = strchr(p, ':');
        if(!p) {
            syslog(LOG_ERR, "iaddr unrecognized data in lease file");
            continue;
        }
        *(p++)='\0';
		...
		*(desc++) = '\0';
		...
		r = upnp_redirect(extif, rhost, eport, iaddr, iport, proto, desc, leaseduration);
		if(r == -1) {
			syslog(LOG_ERR, "Failed to redirect %hu -> %s:%hu protocol %s",
			       eport, iaddr, iport, proto);
		} else if(r == -2) {
			/* Add the redirection again to the lease file */
			lease_file_add(extif, rhost, eport, iaddr, iport, proto_atoi(proto),
			               desc, timestamp);
		}
	}
	fclose(fd);
 ...
 */
int
upnp_redirect(const char * extif, const char * rhost, unsigned short eport,
              ...
              unsigned int leaseduration)
{
	...
	if(!check_upnp_rule_against_permissions(upnppermlist, num_upnpperm,
	                                        eport, address, iport)) {
		syslog(LOG_NOTICE, "redirection permission check failed for "
	 ...
	 */
	rhost_old[0] = '\0';
	r = get_redirect_rule(extif, eport, proto,
	...
	if(r == 0) {
		if(strcmp(iaddr, iaddr_old)==0 &&
		   ((rhost == NULL && rhost_old[0]=='\0') ||
		    ...
		    syslog(LOG_NOTICE, "Updating timestamp to %d and desc to %s", timestamp, desc);

#ifdef ENABLE_LEASEFILE
            if(r == 0) {
                lease_file_remove(ext_if_name, eport, proto);
                lease_file_add(ext_if_name, rhost, eport, iaddr, iport, proto, desc, timestamp);
            }
#endif /* ENABLE_LEASEFILE */
			syslog(LOG_NOTICE, "ignoring redirect request as it matches existing redirect");
			return r;
		} else {

			syslog(LOG_NOTICE, "port %hu protocol %s already redirected to %s:%hu",
				eport, protocol, iaddr_old, iport_old);
			return -2;
		}
	} else {
		timestamp = (leaseduration > 0) ? time(NULL) + leaseduration : 0;
		syslog(LOG_NOTICE, "redirecting port %hu to %s:%hu protocol %s on %s for: %s",
			eport, iaddr, iport, protocol, extif, desc);
		return upnp_redirect_internal(extif, rhost, eport, iaddr, iport, proto,
		                              desc, timestamp);
	}
...
int
upnp_redirect_internal(const char * extif, const char * rhost, unsigned short eport,
                       const char * iaddr, unsigned short iport,
                       ...
                       unsigned int timestamp)
{
	...
	if(add_redirect_rule2(extif, rhost, eport, iaddr, iport, proto,
	                      desc, timestamp) < 0) {
		return -1;
	}

#ifdef ENABLE_LEASEFILE
	lease_file_add(extif, rhost, eport, iaddr, iport, proto, desc, timestamp);
#endif
/*	syslog(LOG_INFO, "creating pass rule to %s:%hu protocol %s for: %s",
		iaddr, iport, protocol, desc);*/
	if(add_filter_rule2(extif, rhost, iaddr, eport, iport, proto, desc) < 0) {
	...
	upnp_event_var_change_notify(extif, EWanIPC);
#endif
	return 0;
}
...
int
upnp_get_redirection_infos(const char * ifname, unsigned short eport, const char * protocol,
                           unsigned short * iport,
                           ...
                           unsigned int * leaseduration)
{
	...
	if(rhost && (rhostlen > 0))
		rhost[0] = '\0';
	r = get_redirect_rule(ifname, eport, proto_atoi(protocol),
                                    ...
                                    unsigned int * leaseduration)
{
	...
	if(rhost && (rhostlen > 0))
		rhost[0] = '\0';
	if(get_redirect_rule_by_index(index, ifname, NULL, 0, eport, iaddr, iaddrlen,
...
int
_upnp_delete_redir(const char * ifname, unsigned short eport, int proto)
{
	int r;
#if defined(__linux__)
	r = delete_redirect_and_filter_rules(ifname, eport, proto);
#else
	r = delete_redirect_rule(ext_if_name, eport, proto);
	delete_filter_rule(ext_if_name, eport, proto);
#endif
#ifdef ENABLE_LEASEFILE
	lease_file_remove( ifname, eport, proto);
#endif

#ifdef ENABLE_EVENTS
	upnp_event_var_change_notify(ifname, EWanIPC);
#endif
	return r;
}

int
upnp_delete_redirection(const char * ifname, unsigned short eport, const char * protocol)
{
	syslog(LOG_INFO, "removing redirect rule port %hu %s", eport, protocol);
	return _upnp_delete_redir(ifname, eport, proto_atoi(protocol));
}

/* upnp_get_portmapping_number_of_entries()
 * TODO: improve this code. */
int
upnp_get_portmapping_number_of_entries(const char *ifname)
{
	int n = 0, r = 0;
	unsigned short eport, iport;
	...
	do {
		...
		r = upnp_get_redirection_infos_by_index(n, ifname, &eport, protocol, &iport,
		                                        iaddr, sizeof(iaddr),
		                                        desc, sizeof(desc),
		                                        rhost, sizeof(rhost),
		                                        &leaseduration);
...
get_upnp_rules_state_list(int max_rules_number_target)
{
	...
	while(get_redirect_rule_by_index(i, "*", ifnamerule, IFNAMSIZ, &tmp->eport, 0, 0,
								  ...
								  &tmp->packets, &tmp->bytes) >= 0)
	{
		...
		snprintf(tmp->ifname, IFNAMSIZ, "%s", ifnamerule);
	...
	for(p = &list, tmp = list; tmp; tmp = *p)
	{
		if(tmp->to_remove)
		{
			syslog(LOG_NOTICE, "remove port mapping %s %hu %s because it has expired",
			        tmp->ifname, tmp->eport, (tmp->proto==IPPROTO_TCP)?"TCP":"UDP");
			_upnp_delete_redir(tmp->ifname, tmp->eport, tmp->proto);
			*p = tmp->next;
			free(tmp);
			i--;
		} else {
...
remove_unused_rules(struct rule_state * list)
{
	...
	while(list)
	{
		                     ...
		                     &packets, &bytes) >= 0)
		{
			...
			{
				_upnp_delete_redir(ifname, list->eport, list->proto);
				n++;
			}
		}
		tmp = list;
 ...
 * mapping exists */
unsigned short *
upnp_get_portmappings_in_range(const char * ifname,
                               ...
                               unsigned int * number)
{
	...
	if(!number)
		return NULL;
	return get_portmappings_in_range(ifname, startport, endport, proto, number);
}
