...
typedef enum {
    ...
    MMCONN_TYPE_NULL,       /**< Null connection. */
...
typedef struct {
    ...
    uint8_t ingressSsrc[4];           /**< RTP SSRC for incoming media */
    int     ingressRtpMediaReported;  /**< Indication whether the ingress RTP media start has been reported */
    int     mediaTimeout;             /**< Indication whether we are currently receiving media (0) or not (1) */
} MmConnStats;
 ...
 *
 * \return ::MmPbxError
 * \retval MMPBX_ERROR_NOERROR The command has been forwarded to the DSP
 * \retval MMPBX_ERROR_NOTIMPLEMENTED The DSP did not register a call-back
 * \todo Add other possible return values after implementation.
 */
