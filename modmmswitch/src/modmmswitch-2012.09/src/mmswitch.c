...
static int mmSwitchWriteSocketThreadFunc(void *thread_data)
{
    ...
    while (1) {
        ...
        while (1) {
            ...
            struct kvec             vec;
            struct msghdr           msg;
            struct cmsghdr          *cmsg = NULL; /* Ptr to ancillary hdr  */
            int                     tos   = 0;
            ...
            if (sockQueueEntry->mmSwitchSock->sock != NULL) {
                ...
                if (sockQueueEntry->dst.ss_family == AF_INET) {
                    struct sockaddr_in *addr = (struct sockaddr_in *) &sockQueueEntry->dst;
                    if (addr->sin_addr.s_addr == htonl(INADDR_LOOPBACK)) {
                        /*
                         * Force IPTOS and DONTROUTE for QoS reasons
                         * */
                        tos           = IPTOS_LOWDELAY | IPTOS_PREC_CRITIC_ECP | IPTOS_THROUGHPUT;
                        msg.msg_flags = msg.msg_flags | MSG_DONTROUTE;
                ...
                /* If for some reason the function kernel_sendmsg returns an error, then wait some time (wait for 25 packets) then retry to send.
                 */
...
static void __exit mmSwitchExit(void)
{
    ...
    err = mmConnRtcpStatsDeinit();
    if (err != MMPBX_ERROR_NOERROR) {
        MMPBX_TRC_ERROR("mmConnRtcpStatsDeinit failed with: %s\n", mmPbxGetErrorString(err));
    }
