...
static bp_pinmux_defs_t g_pinmux_defs_0[] = {
  ...
  { bp_usSpiSlaveSelectNum, 0, 48, 93 | BP_PINMUX_VAL_3}, 
...
int BpGetBoardId( char *pszBoardId )
{
    ...
    /* TCH truncate BoardId. => root boardId will be used by voice/dect to lookup parms */
    if (pszBoardId[6] == '_') {
       pszBoardId[6] ='\0';
    }
...
/**************************************************************************
*  Name       : BpGetTxDisableGpio
*
*  Description: This funtion returns the GPIO pin assignments for the SFP
*               Module Tx Disable.
*
*  Parameters : [OUT] sfpGpio - Address of short word that the SFP Tx Disable
...
*                   for the board.
****************************************************************************/
int BpGetSfpTxDisableGpio( unsigned short *sfpTxDisableGpio)
{
    *sfpTxDisableGpio = BP_NOT_DEFINED;
    return BpGetGpio (bp_usGpioSfpTxDisable, sfpTxDisableGpio);
} /* BpGetSfpTxDisableGpio */
...
/**************************************************************************
 * Name       :BpGetLanWanMuxSwPort
 *
 * Description: This function get the switch port whose ext crossbar port
 *              is configured to support both LAN or WAN
 * Parameters : [IN] unit - specify to get the runner port or switch port
 *              [OUT] switchPort - the returned switch port
 * Returns    : BP_SUCCESS - Success, the switch port is found
 *              BP_VALUE_NOT_DEFINED - Error, the switch port is not found
 **************************************************************************/
int BpGetLanWanMuxSwPort(int unit, int* switchPort)
{
    ETHERNET_MAC_INFO localEnetInfo[BP_MAX_ENET_MACS];
    PETHERNET_MAC_INFO pRunner = NULL;
    PETHERNET_MAC_INFO pSwitch = NULL;
    int cb_port;

    if((unit < 0) || (unit >= BP_MAX_ENET_MACS))
        goto NotFound;

    BpGetEthernetMacInfo(localEnetInfo, BP_MAX_ENET_MACS);
    pRunner = &localEnetInfo[0];
    pSwitch = &localEnetInfo[1];

    for ( cb_port = 0; cb_port < BP_MAX_CROSSBAR_EXT_PORTS; cb_port++ )
    {
        if (BP_IS_CROSSBAR_PORT_DEFINED(pRunner->sw, cb_port) &&
            BP_IS_CROSSBAR_PORT_DEFINED(pSwitch->sw, cb_port))
        {
            if(unit == 0)
                *switchPort = pRunner->sw.crossbar[cb_port].switch_port;
            else if(unit == 1)
                *switchPort = pSwitch->sw.crossbar[cb_port].switch_port;
            return BP_SUCCESS;
        }
    }

NotFound:
    *switchPort = BP_CROSSBAR_NOT_DEFINED;
    return BP_VALUE_NOT_DEFINED;
}
EXPORT_SYMBOL(BpGetLanWanMuxSwPort);
...
/*
 * Name       : BpGetLanWanMuxGpio
 *
 * Description: This function returns the GPIO pin assignment for the Mux of
 *              wan/lan port.
 *
 * Parameters : [OUT] pusValue - Address of short word that the Mux
 *                  GPIO pin is returned in.
 *
 * Returns    : BP_SUCCESS - Success, value is returned.
 *              BP_BOARD_ID_NOT_SET - Error, BpSetBoardId has not been called.
...
int BpGetLanWanMuxGpio( unsigned short *pusValue )
{
    return( BpGetUs(bp_usGpioLanWanMux, pusValue ) );
}

EXPORT_SYMBOL(BpGetLanWanMuxGpio);
...
/**************************************************************************
 * * Name : BpGetTriplexerTrx
 * *
 * * Description: This function returns true if triplexer trx is used in platform.
 * *
 * * Parameters : [OUT] pusValue - Address of short word that the
 * * value is returned in.
 * *
 ...
 * * for the board.
 * ***************************************************************************/
int BpGetTriplexerTrx( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usTriplexerTrx, pusValue ) );
} /* BpGetTriplexerTrx */
#endif
...
int BpGetCustomATAG_tch( void *pcpValue )
{
	return( BpGetCp(bp_cpCustomATAG_tch, pcpValue ) );

}
int BpGetVethPortmap_tch(int *n, char *cpVethPortmap)
{
    char *bp_cpVethPortmap;
    int i;

    if (*n < BP_MAX_VETH_PORTS
        || BP_SUCCESS != BpGetCp(bp_cpVethPortmap_tch, &bp_cpVethPortmap)) {
        return(-1);
    }

    *n = BP_MAX_VETH_PORTS;
    for (i = 0; i < BP_MAX_VETH_PORTS; i++) {
        cpVethPortmap[i] = bp_cpVethPortmap[i];
    }
    return(0);   
}
...
/**************************************************************************
* Name       : BpGetLedGpioTch
*
* Description: This function returns the GPIO for the specified led
*
* Parameters : [IN] led - LED
*              [OUT] gpio - Address of short word that the
*                  value is returned in.
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_VALUE_NOT_DEFINED - Failure
***************************************************************************/

int BpGetLedGpio_tch(enum tch_leds led, unsigned short * gpio) {
	void  *token = NULL;
	enum bp_id id;



	/* Have to maintain a mapping between kLed and bp_us since Broadcom considers
	bp_usGpio as private API.  We provide kLed as public API */
	switch (led) {
	case kLedPowerRed:
		id = bp_usGpioLedPowerRed_tch;
		break;
	case kLedPowerGreen:
		id = bp_usGpioLedPowerGreen_tch;
		break;
	case kLedPowerBlue:
		id = bp_usGpioLedPowerBlue_tch;
		break;
	case kLedPowerWhite:
		id = bp_usGpioLedPowerWhite_tch;
		break;
	case kLedBroadbandGreen:
		id = bp_usGpioLedBroadbandGreen_tch;
		break;
	case kLedBroadbandRed:
		id = bp_usGpioLedBroadbandRed_tch;
		break;
	case kLedBroadband2Green:
		id = bp_usGpioLedBroadband2Green_tch;
		break;
	case kLedBroadband2Red:
		id = bp_usGpioLedBroadband2Red_tch;
		break;
	case kLedDectGreen:
		id = bp_usGpioLedDectGreen_tch;
		break;
	case kLedDectBlue:
		id = bp_usGpioLedDectBlue_tch;
		break;
	case kLedDectRed:
		id = bp_usGpioLedDectRed_tch;
		break;
	case kLedEthernetGreen:
		id = bp_usGpioLedEthernetGreen_tch;
		break;
	case kLedEthernetRed:
		id = bp_usGpioLedEthernetRed_tch;
		break;
	case kLedEthernetBlue:
		id = bp_usGpioLedEthernetBlue_tch;
		break;
	case kLedIPTVGreen:
		id = bp_usGpioLedIPTVGreen_tch;
		break;
	case kLedWirelessGreen:
		id = bp_usGpioLedWirelessGreen_tch;
		break;
	case kLedWirelessRed:
		id = bp_usGpioLedWirelessRed_tch;
		break;
	case kLedWirelessBlue:
		id = bp_usGpioLedWirelessBlue_tch;
		break;
	case kLedWirelessWhite:
		id = bp_usGpioLedWirelessWhite_tch;
		break;
	case kLedWireless5GHzGreen:
		id = bp_usGpioLedWireless5GHzGreen_tch;
		break;
	case kLedWireless5GHzRed:
		id = bp_usGpioLedWireless5GHzRed_tch;
		break;
	case kLedInternetGreen:
		id = bp_usGpioLedInternetGreen_tch;
		break;
	case kLedInternetRed:
		id = bp_usGpioLedInternetRed_tch;
		break;
	case kLedInternetBlue:
		id = bp_usGpioLedInternetBlue_tch;
		break;
	case kLedVoip1Green:
		id = bp_usGpioLedVoip1Green_tch;
		break;
	case kLedInternetWhite:
		id = bp_usGpioLedInternetWhite_tch;
		break;
	case kLedVoip1Red:
		id = bp_usGpioLedVoip1Red_tch;
		break;
	case kLedVoip1Blue:
		id = bp_usGpioLedVoip1Blue_tch;
		break;
	case kLedVoip1White:
		id = bp_usGpioLedVoip1White_tch;
		break;
	case kLedVoip2Green:
		id = bp_usGpioLedVoip2Green_tch;
		break;
	case kLedVoip2Red:
		id = bp_usGpioLedVoip2Red_tch;
		break;
	case kLedWPSGreen:
		id = bp_usGpioLedWPSGreen_tch;
		break;
	case kLedWPSRed:
		id = bp_usGpioLedWPSRed_tch;
		break;
	case kLedWPSBlue:
		id = bp_usGpioLedWPSBlue_tch;
		break;
	case kLedUsbGreen:
		id = bp_usGpioLedUsbGreen_tch;
		break;
	case kLedUpgradeBlue:
		id = bp_usGpioLedUpgradeBlue_tch;
		break;
	case kLedUpgradeGreen:
		id = bp_usGpioLedUpgradeGreen_tch;
		break;
	case kLedLteGreen:
		id = bp_usGpioLedLteGreen_tch;
		break;
	case kLedLteRed:
		id = bp_usGpioLedLteRed_tch;
		break;
	case kLedLteBlue:
		id = bp_usGpioLedLteBlue_tch;
		break;
	case kLedLteWhite:
		id = bp_usGpioLedLteWhite_tch;
		break;
	...
	case kLedMobileGreen:
		id = bp_usGpioLedMobileGreen_tch;
		break;
	case kLedMobileRed:
		id = bp_usGpioLedMobileRed_tch;
		break;
	case kLedMobileBlue:
		id = bp_usGpioLedMobileBlue_tch;
		break;
	case kLedMobileWhite:
		id = bp_usGpioLedMobileWhite_tch;
		break;
	case kLedMoCAGreen:
		id = bp_usGpioLedMoCAGreen_tch;
		break;
	case kLedSFPGreen:
		id = bp_usGpioLedSFPGreen_tch;
		break;
	case kLedAmbient1White:
		id = bp_usGpioLedAmbient1White_tch;
		break;
	case kLedAmbient2White:
		id = bp_usGpioLedAmbient2White_tch;
		break;
	case kLedAmbient3White:
		id = bp_usGpioLedAmbient3White_tch;
		break;
	case kLedAmbient4White:
		id = bp_usGpioLedAmbient4White_tch;
		break;
	case kLedAmbient5White:
		id = bp_usGpioLedAmbient5White_tch;
		break;
	default:
		printk("Unsupported LED id: %d\n",  led);
		return BP_VALUE_NOT_DEFINED;
	}

	return BpEnumUs(id, &token, gpio);
}


/**************************************************************************
* Name       : BpGetExtSwLedCfg_tch
*
* Description: This function returns whether the external switches needs
*              a specific Led Config Override
*
* Parameters : [OUT] override - Value of the LED override
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_VALUE_NOT_DEFINED - Failure
***************************************************************************/

int BpGetExtSwLedCfg_tch(unsigned short * value) {
    return( BpGetUs(bp_usExtSwLedCfg_tch, value ) );
} /* BpGetExtSwLedCfg_tch */


/**************************************************************************
* Name       : BpGetRxRgmiiClockDelayAtMac_tch
*
* Description: Indicates if the board diable the RGMII skew
*
*
* Parameters : [OUT] pusValue - 1 if RGMII Skew is disabled 
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_BOARD_ID_NOT_SET - Error, BpSetBoardId has not been called.
*              BP_VALUE_NOT_DEFINED - Not defined
***************************************************************************/
int BpGetRxRgmiiClockDelayAtMac_tch( unsigned short *pusValue )
{
    return( BpGetUs(bp_usRxRgmiiClockDelayAtMac_tch, pusValue ) );
} /* BpGetRxRgmiiClockDelayAtMac_tch */


/**************************************************************************
* Name       : BpGetHardRSTGpio_tch
*
* Description: Indicates if the board use hard reset during reboot
*
* Parameters : [OUT] pusValue - Address of short word that the
*                  gpio number is returned in.
*
...
*
***************************************************************************/
int BpGetHardRSTGpio_tch( unsigned short *pusValue )
{
    return( BpGetGpio(bp_usGpioHardRST_tch, pusValue ) );
} /* BpGetHardRSTGpio_tch */

/*****************************************************************************
 * Name       : BpGetPcieOrder_tch
 *
 * Description: This function returns the PCIE boot order.
 *
 * Parameters : [OUT] pusValue - Address of short word that the PCIE boot
 *                   order is returned in.
 *
 * Returns    : BP_SUCCESS - Success, value is returned.
 *              BP_BOARD_ID_NOT_SET - Error, BpPcieOrder_tch has not been called.
 *              BP_VALUE_NOT_DEFINED - At least one return value is not defined
 *                  for the board.
 ****************************************************************************/
int BpGetPcieOrder_tch( unsigned short *pusValue )
{
    return( BpGetUl(bp_ulPcieOrder_tch, pusValue ) );
}

EXPORT_SYMBOL(BpGetPcieOrder_tch);
/* *** */
/**************************************************************************
* Name       : BpGetAELinkLed_tch
*
* Description: Get the configuartion for the AE Link/ACT LED
*              In future we can define a special Link/ACT LED for AE interface to separate with GPHY4 LED, such as GPHY4 can use GPIO 14,
*              and AE can use GPIO 22, vice versa. We will write new code to do dynamic switch when AE or GPHY4 is connected to runner
*
* Parameters : [OUT] pusValue - GPIO definition for AE Link/ACT LED
*              BP_GPIO_NONE: there is no AE Link/ACT LED
*              BP_GPIO_14_Ax: AE Link/ACT LED is connected to GPIO 14
*              BP_GPIO_22_Ax: AE Link/ACT LED is connected to GPIO 22
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_BOARD_ID_NOT_SET - Error, BpSetBoardId has not been called.
*              BP_VALUE_NOT_DEFINED - At least one return value is not defined
...
*
***************************************************************************/
int BpGetAELinkLed_tch( unsigned short *pusValue )
{
    return( BpGetUs(bp_usAELinkLed_tch, pusValue ) );
} /* BpGetAELinkLed_tch */

/**************************************************************************
* Name       : BpGetusLinkLed_tch
*
* Description: Get the bp_usLinkLed for the specified PhyType and PhyId
*
* Parameters : [OUT] pusValue - GPIO definition for specified GPHY Link/ACT LED
*              [IN] PhyTypeId - the specified PhyType Id
*              [IN] PhyId - the specified PhyId
*
* Returns    : BP_SUCCESS - Success, value is returned.
*              BP_BOARD_ID_NOT_SET - Error, BpSetBoardId has not been called.
*              BP_VALUE_NOT_DEFINED - At least one return value is not defined
...
*
***************************************************************************/
int BpGetusLinkLed_tch(unsigned short *pusValue, unsigned char PhyTypeId, unsigned char PhyId)
{
    enum bp_id bp_aucPhyType[BP_MAX_ENET_MACS+1] = {bp_ucPhyType0, bp_ucPhyType1, bp_last};
    ...
    bp_elem_t *PhyTypeElem, *PhyIdElem, *tempElem;
    int nRet = BP_VALUE_NOT_DEFINED;

    if((PhyTypeId >= BP_MAX_ENET_MACS) || (PhyId >= BP_MAX_SWITCH_PORTS)) {
        printk("BpGetusLinkLed_tch:invalid input parameter,PhyTypeId=%d,PhyId=%d \n", PhyTypeId, PhyId);
        return nRet;
    }

    tempElem = 0;
    PhyTypeElem = BpGetElem(bp_aucPhyType[PhyTypeId], &tempElem, bp_last);
    if(PhyTypeElem->id == bp_last) {
        printk("BpGetusLinkLed_tch:PhyType elem not found\n");
        return nRet;
    }
    PhyIdElem = BpGetElem(bp_aulPhyId[PhyId], &PhyTypeElem, bp_aucPhyType[PhyTypeId+1]);
    if(PhyIdElem->id == bp_last) {
        printk("BpGetusLinkLed_tch:PhyId elem not found\n");
        return nRet;
    }

    *pusValue = BpGetSubUs(bp_usLinkLed, PhyIdElem, bp_aulPhyId[PhyId+1]);
    nRet = (BP_NOT_DEFINED != *pusValue ? BP_SUCCESS : BP_VALUE_NOT_DEFINED);
...
/* Technicolor added start */
int BpGrepPinmuxByPortandGpio_tch(enum bp_id id, unsigned short portnum, unsigned short gpionum, unsigned short *pusFunction, unsigned long *pulMuxInfo)
{
    int i;

    if (0 == g_pinmux_defs) {
        printk("ERROR:BpGrepPinmuxByPortandGpio called before pinmux table selected\n");
        return(BP_VALUE_NOT_DEFINED);
    }

    for (i = 0; g_pinmux_defs[i].id != bp_last; i++) {
        if ((g_pinmux_defs[i].id == id) && (g_pinmux_defs[i].port == portnum)) {
            if ((g_pinmux_defs[i].func == gpionum) || (g_pinmux_defs[i].func == -1)) {
                *pusFunction = g_pinmux_defs[i].func;
                *pulMuxInfo = g_pinmux_defs[i].mux_info | BP_PINMUX_VALID;
                if (g_pinmux_defs[i].func == -1) {
                    *pulMuxInfo |= (gpionum & BP_GPIO_NUM_MASK);
...
EXPORT_SYMBOL(BpGrepPinmuxByPortandGpio_tch);
/* Technicolor added end */
...
#if defined(_BCM96838_) || defined(CONFIG_BCM96838) || defined(_BCM96848_) || defined(CONFIG_BCM96848) || defined(_BCM96858_) || defined(CONFIG_BCM96858) || defined(CONFIG_BCM96836) || defined(_BCM96836_)
/****************************************************************************
