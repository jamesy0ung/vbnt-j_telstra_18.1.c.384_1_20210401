...
extern int cfe_load_image(const char* fname, void* ddr_addr, unsigned long max_size);
#endif
...
int GetRCalSetting(int resistor, int* rcal)
{
	...
	return rc;
#endif /* defined(CONFIG_BCM963381) */
}
#endif
...
void pmc_initmode(void)
{
    ...
    if ( (PERF->RevID & REV_ID_MASK) == 0xb0 )
    {
...
int pmc_boot(unsigned long addr)
{
	const unsigned long ddr = addr;
	const unsigned rom = 0x1fc00000;
	const unsigned align =  ~((1 << (ffs(addr) - 1)) - 1);
	int sleep_us = 0;

	// reset m4ke
	PMC->ctrl.softResets = 1;
	// set boot vector to data buffer
	PMC->ctrl.addr1WndwMask = align;
	PMC->ctrl.addr1WndwBaseIn = rom;
	PMC->ctrl.addr1WndwBaseOut = ddr;

	cfe_usleep(500);		// ensure in reset for 500
	PMC->ctrl.softResets = 0;	// ureset m4ke

	// wait upto 400ms for pmc to initialize
	while ((sleep_us += 5) < 400 * 1000) {
		cfe_usleep(5);
		// Check if power on rail 0 is OK
		if (((PMC->ctrl.hostMboxIn >> 24) & 7) == 2) {
			// manually set pmc mode
			pmc_mode = PMC_MODE_DQM;
			// Check, if it can hanldle a DQM command
			SendCommand(cmdPing, 0, 0, 0, 0, 0);
			return kPMC_NO_ERROR;
		}
	}

	PMC->ctrl.softResets = 1;	// reset m4ke
	return kPMC_BOOT_FAILED;
}
#endif

#if defined (_BCM96836_) && defined(CFG_RAMAPP)
#include "lib_types.h"
...
static void pmc_boot(void)
{
    ...
    // first word in pmcappdata is the length of image.
    len = (pmcappdata[0]<<24) | (pmcappdata[1]<<16) | (pmcappdata[2]<<8) | pmcappdata[3];
    ...
    memcpy((void*)physAddrPmc, pmcappdata+4, len);
...
# if (INC_NAND_FLASH_DRIVER == 1) || (INC_SPI_NAND_DRIVER == 1) || (INC_SPI_PROG_NAND == 1)
	// PMC needs to be booted using the code in the flash from CFE only.
	// Load PMC binary at reserved PMC boot address
	if ((rc = cfe_load_image ("pmc.bin", (void*)CFG_BOOT_PMC_ADDR, CFG_BOOT_PMC_SIZE )) == 0) {
		printk ("Success: %s PMC to load\n", __func__);
		// Boot PMC using the code loaded at PMC boot address
		if ((rc = pmc_boot(CFG_BOOT_PMC_ADDR)) == kPMC_BOOT_FAILED)
			printk ("Error: %s PMC failed to boot\n", __func__);
	}
	else
		printk ("Error: %s PMC failed to load\n", __func__);
# endif
#endif /* _BCM963158_ */
