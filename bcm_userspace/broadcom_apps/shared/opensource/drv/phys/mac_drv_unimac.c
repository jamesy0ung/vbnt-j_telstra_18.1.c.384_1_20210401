...
static int port_unimac_init(mac_dev_t *mac_dev)
{
    ...
    mac_hwapi_set_unimac_cfg(emac, (uintptr_t)mac_dev->priv);
...
static int port_unimac_cfg_get(mac_dev_t *mac_dev, mac_cfg_t *mac_cfg)
{
    ...
    mac_cfg->pause_rx = emac_cfg.rx_flow_control ? 1 : 0;
    mac_cfg->pause_tx = emac_cfg.tx_flow_control ? 1 : 0;
...
static int port_unimac_cfg_set(mac_dev_t *mac_dev, mac_cfg_t *mac_cfg)
{
    ...
    emac_cfg.rx_flow_control = mac_cfg->pause_rx ? 1 : 0;
    emac_cfg.tx_flow_control = mac_cfg->pause_tx ? 1 : 0;
