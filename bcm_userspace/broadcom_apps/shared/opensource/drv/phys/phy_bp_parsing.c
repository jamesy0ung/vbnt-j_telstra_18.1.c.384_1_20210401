...
static void bp_parse_rgmii_delay(const ETHERNET_MAC_INFO *emac_info, uint32_t port, phy_dev_t *phy_dev)
{
    uint32_t phy_id = emac_info->sw.phy_id[port];
    uint32_t intf = phy_id & MAC_IFACE;
...
phy_dev_t *bp_parse_phy_dev(const ETHERNET_MAC_INFO *emac_info, uint32_t port)
{
    ...
    addr = bp_parse_phy_addr(emac_info, port);
    ...
    bp_parse_rgmii_delay(emac_info, port, phy_dev);
    bp_parse_aux_phy(emac_info, port, phy_dev);
