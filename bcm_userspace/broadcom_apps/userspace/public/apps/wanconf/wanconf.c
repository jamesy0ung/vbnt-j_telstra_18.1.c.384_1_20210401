...
int load_gpon_modules(rdpa_wan_type wan_type)
{
    ...
    {
        rc = insmod("/lib/modules/"KERNELVER"/gponstack.ko");
    }
    else if (wan_type == rdpa_wan_xgpon)
    {
        rc = insmod("/lib/modules/"KERNELVER"/ngponstack.ko");
    }

    rc = insmod("/lib/modules/"KERNELVER"/bcmgpon.ko");
...
int create_bcmenet_vport(char *ifname, int op)
{
    struct ifreq ifr;
    int err, skfd;
...
static void gbeSendPostMdmMsg(void *msgHandle)
{
...
int main(int argc __attribute__((unused)), char *argv[] __attribute__((unused)))
{
    ...
    if (!strcmp(buf, RDPA_WAN_TYPE_VALUE_GPON))
...
#define PSP_RATE_STR_LEN 4
        ...
        if ((count >= PSP_RATE_STR_LEN) && 
            (!strncmp(&buf[RATE_STR_LEN], RDPA_WAN_RATE_10G, RATE_STR_LEN)))
        {
    ...
    else if (!strcmp(buf, RDPA_WAN_TYPE_VALUE_GBE))
    {
    ...
    else if (!strcmp(buf, RDPA_WAN_TYPE_VALUE_XGPON1) || !strcmp(buf, RDPA_WAN_TYPE_VALUE_NGPON2) || !strcmp(buf, RDPA_WAN_TYPE_VALUE_XGS))
    ...
    switch (wan_type)
    {
        ...
        case rdpa_wan_xepon_ae:
            ...
            if(launch_app == 0)
            {
                ...
                rc = create_bcmenet_vport(ifname, ETHCREATEEPONAEVPORT);
        ...
        case rdpa_wan_gbe:
            {
                ...
                if (ifname[0] == '\0')
                    rc = create_bcmenet_vport(ifname, ETHINITWAN);
