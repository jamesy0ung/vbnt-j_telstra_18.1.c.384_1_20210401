  ...
  CAL_CRC32_CB calCrc32Cb)
{
    ...
    mtdP = get_mtd_device_handle(MTD_NAME_IMAGE, &mtdFd);
    if (mtdP == 0)
    {
    ...
    {
        put_mtd_device(mtdP, mtdFd);
    }

    mtdP = get_mtd_device_handle(MTD_NAME_IMAGE_UPDATE, &mtdFd);
    if (mtdP == NULL)
    {
...
int imgif_nand_close(IMGIF_HANDLE h, UBOOL8 abortFlag)
{
    ...
    {
        if (handleCferom(ctxP->mtdP, (char*)ctxP->cferomP, ctxP->wfiFlags,
          ctxP->nvramDataP) < 0)
        {
...
static imgif_ret_e imgif_ctx_clean(imgif_ctx_t *ctxP)
{
    ...
    if (ctxP->mtdP != NULL)
    {
        put_mtd_device(ctxP->mtdP, ctxP->mtdFd);
        ctxP->mtdP = NULL;
        ctxP->mtdFd = 0;
    }
