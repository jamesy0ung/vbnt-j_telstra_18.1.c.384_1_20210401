...
static int scan_partition(char * name)
{
    ...
    mtd = get_mtd_device_handle(name, &mtd_fd);
...
int nandReadNvramData(void *nvramData)
{
    ...
    if ( (mtd = get_mtd_device_handle("nvram", &mtd_fd)) == 0)
    {
...
{ /* NAND Flash */
    ...
    if (type == UBI_IMAGE)
    {
        ...
        if( partitionIsBootedPartition ) // get from booted image
            mtd = get_mtd_device_handle("image", &mtd_fd);
        else // get from non-booted image
            mtd = get_mtd_device_handle("image_update", &mtd_fd);
...
static int bcmNandImageSet( char *rootfs_part, unsigned char *image_ptr, int img_size, NVRAM_DATA * inMemNvramData_buf)
{
    ...
    int mtd0_fd;
    mtd_info_t * mtd0 = get_mtd_device_handle("image", &mtd0_fd);
    ...
    if( !IS_ERR_OR_NULL(mtd0) )
    {
        ...
        {
            put_mtd_device(mtd0, mtd0_fd);
            mtd0 = get_mtd_device_handle(rootfs_part, &mtd0_fd);
    ...
    if(get_flash_type() == FLASH_IFC_NAND ) {
        if(nand_image_type(image_ptr) == 0)
        {
            ...
            if( (wt.wfiFlags&WFI_FLAG_HAS_PMC) == 0 )
            {
                ...
                if( *pImg == 0 && *(pImg+1) == 0 && *(pImg+2) == 0 && *(pImg+3) == 0 )
                {
                    ...
                    mtd1 = get_mtd_device_handle("nvram", &mtd1_fd);
...
#if defined(CONFIG_BCM96838) || defined(CONFIG_BCM963268) || defined(CONFIG_BCM963381) || defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148) || defined(CONFIG_BCM96848) || defined(CONFIG_BCM94908) || defined(CONFIG_BCM96858) || defined(CONFIG_BCM96836) || defined(CONFIG_BCM963158)
        if ((wt.wfiFlags & WFI_FLAG_SUPPORTS_BTRM) && (ofs != 0))
        {
            ...
            int mtd1_fd;
            mtd_info_t *mtd1 = get_mtd_device_handle("nvram", &mtd1_fd);
            if( !IS_ERR_OR_NULL(mtd1) )
            {
...
int handleCferom(mtd_info_t *mtd, char *image_ptr, unsigned int wfiFlags, void *inMemNvramData)
{
    int nvramXferSize;
...
int flashCferom(unsigned char *image_ptr, unsigned int size)
{
    /* Prepare to flash the CFE ROM boot loader. */
    int mtd_fd;
    mtd_info_t *mtd = get_mtd_device_handle("nvram", &mtd_fd);
    int address = 0;
    int status = 0;
...
int nandCommit( int partition, char *string )
{
    ...
    for (i = 0; i < 2; i++)
    {
        if (parse_ubi(0, buf, 0, mtd->size / len, len, i ? VOLID_METADATA_COPY : VOLID_METADATA, "committed", string, 0, 0, read_blk, (*string != 0) ? write_blk : 0, (*string != 0) ? erase_blk : 0, mtd, mtd_fd) == 1)
        {
