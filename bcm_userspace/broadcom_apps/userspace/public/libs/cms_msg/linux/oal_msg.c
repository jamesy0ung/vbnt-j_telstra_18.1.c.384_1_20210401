...
CmsRet oalMsg_receive(SINT32 fd, CmsMsgHeader **buf, UINT32 *timeout)
{
...

   {
      ...
      SINT32 totalRemaining=sizeof(CmsMsgHeader);
      char *inBuf;

      inBuf = (char *)msg;

      while (totalReadSoFar < sizeof(CmsMsgHeader))
      {
          if (timeout)
          {
          ...
          if (rc < 0 && (11 == errno || 4 == errno) /*(EAGAIN == errno || EINTR == errno)*/)
          {
             //cmsLog_error("read < 0; errno=%d, read again", errno);
             continue;
          }
          else if (rc <= 0)  /* new 2.6.21 kernel seems to give us this before rc==0 */
          {
             /* broken connection */
             cmsMem_free(msg);
             return CMSRET_DISCONNECTED;
