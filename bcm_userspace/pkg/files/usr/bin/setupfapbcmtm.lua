...
local function configure_ethdevice(ethdevice, classgroup, type)
   ...
   if trafficdescriptor ~= nil and bool_is_true(cursor:get("ethernet", trafficdescriptor, "enable"), 1) then
      ...
      local mbr = cursor:get("ethernet", trafficdescriptor, "max_bit_rate")
      local mbs = cursor:get("ethernet", trafficdescriptor, "max_burst_size")
...
for ethdev, classgroup in pairs(parse_ethdev_config_type(arg[1])) do
   ...
   cursor_state:revert("ethernet", ethdev, "QoS_classgroup")
   if classgroup then
      configure_ethdevice(ethdev, classgroup, arg[1])
      cursor_state:set("ethernet", ethdev, "QoS_classgroup", classgroup) --used by setuptm
