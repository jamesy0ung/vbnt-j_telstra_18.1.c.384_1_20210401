...
static int spu_runner_rxq_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        while ( count < SPU_RX_BUDGET )
        {
            ...
            if ( (rc != 0) || (NULL == pNBuf) )
            {
                SPU_TRACE(("spu_runner_rxq_thread: error in rdpa_cpu_loopback_packet_get() (%d)\n", rc));
                continue;
            }

            if ( info.reason != rdpa_cpu_rx_reason_direct_queue_2 )
            {
                SPU_TRACE(("spu_runner_rxq_thread: received unexpected packet\n"));
                nbuff_free(pNBuf);
                continue;
            }
...
static int spu_runner_cfg_cpu_reason_to_queue(void)
{
   ...
   {
      if ( reason_cfg_idx.reason == rdpa_cpu_rx_reason_direct_queue_2 )
      {
