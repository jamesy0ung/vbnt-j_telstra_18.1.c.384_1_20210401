...
#endif // !defined(CONFIG_BCM_TCH_BL)
#include <bcmTag.h>
#include <board.h>
...
#ifdef CONFIG_BCM_CLINK_BL
    { name: "rootfs_data",   offset: 0,           size: 0 },
#else
    { name: CONFIG_BCM_DATA_PARTITION_NAME,          offset: 0,           size: 0 },
#endif
...
#ifdef CONFIG_BCM_CLINK_BL
    bcm63XX_nand_parts[nr_parts].name = "persist";
    bcm63XX_nand_parts[nr_parts].offset = nvram.ulNandPartOfsKb[NP_DATA+1] * 1024;
    bcm63XX_nand_parts[nr_parts].size = nvram.ulNandPartOfsKb[NP_DATA+1] * 1024;
    bcm63XX_nand_parts[nr_parts].ecclayout = mtd->ecclayout;
...
#ifdef CONFIG_BCM_CLINK_BL
void kerSysWriteNvRam(PNVRAM_DATA pNvramData)
{
    if (pNvramData == NULL)
        return;
...
EXPORT_SYMBOL(kerSysWriteNvRam);
#endif
