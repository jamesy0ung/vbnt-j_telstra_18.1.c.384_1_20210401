...
#if defined(CONFIG_BCM96838) || (defined(CONFIG_BCM963268) && defined(CONFIG_MMPBX_API_PATCH))
    {
        uint32 int_status, i;
...
static int __used map_hw_timer_interrupt(unsigned long interrupt_id)
{
...
int ext_timer_set_affinity(EXT_TIMER_NUMBER timer_num, unsigned int cpuId, int force)
{
    ...
    ret = __irq_set_affinity(irq, &mask, force);
