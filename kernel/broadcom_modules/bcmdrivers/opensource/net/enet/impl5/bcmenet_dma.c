...
#if ENET_RX_CHANNELS_MAX > 1
int weight_pkts[ENET_RX_CHANNELS_MAX] = {[0] = 8, [1 ... (ENET_RX_CHANNELS_MAX-1)] = 1};
int pending_weight_pkts[ENET_RX_CHANNELS_MAX] = {[0] = 8, [1 ... (ENET_RX_CHANNELS_MAX-1)] = 1};
#else
int weight_pkts[ENET_RX_CHANNELS_MAX] = {32};
int pending_weight_pkts[ENET_RX_CHANNELS_MAX] = {32};
#endif
...
int bcmeapi_ioctl_ethsw_wrrparam(struct ethswctl_data *e)
{
    ...
    if (e->type == TYPE_GET) {
        e->max_pkts_per_iter = max_pkts;
        memcpy(e->weights, weights, sizeof(e->weights));
    } else {
