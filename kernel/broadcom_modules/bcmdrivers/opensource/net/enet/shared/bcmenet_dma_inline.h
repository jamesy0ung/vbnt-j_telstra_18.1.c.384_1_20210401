...
static inline int bcmeapi_pkt_xmt_dispatch(EnetXmitParams *pParam)
{
	...
	if(!pFkb) {	/* skb */
        ...
        // then we can shorten the flush length.
        if (dirty_p) {
/* FRV: WLAN driver can also set dirty pointer.  FC can shorten the buffer in case
        it was a padded eth frame (64 byte min) -> change dirty_p */
#ifdef CONFIG_TCH_KF_WLAN_PERF_RX_ENET
            if (dirty_p > pEnd)
                dirty_p = pEnd;
            if (dirty_p < skbp->head)
#else 
...
#ifdef CONFIG_TCH_KF_WLAN_PERF_RX_ENET
extern RecycleFuncP bcm63xx_eth_wl_recycle_hook;
extern void (*bcm63xx_eth_wl_recycle_fast)(struct sk_buff *skb);
#endif /* CONFIG_TCH_KF_WLAN_PERF_RX_ENET */ 
/* FRV: Direct release of buffers from eth back to wl */
...
/* FRV: Direct release of buffers from eth back to wl */
#ifdef CONFIG_TCH_KF_WLAN_PERF_RX_ENET
        if (bcm63xx_eth_wl_recycle_hook && IS_SKBUFF_PTR(txRecycle_p->key) )
        {
            void *pBuf = PNBUFF_2_PBUF(txRecycle_p->key);
            struct sk_buff *skb = (struct sk_buff *)pBuf;

            if (skb->recycle_hook == bcm63xx_eth_wl_recycle_hook) {
                bcm63xx_eth_wl_recycle_fast(skb);
                continue;
            }
        }
#endif /* CONFIG_TCH_KF_WLAN_PERF_RX_ENET */
