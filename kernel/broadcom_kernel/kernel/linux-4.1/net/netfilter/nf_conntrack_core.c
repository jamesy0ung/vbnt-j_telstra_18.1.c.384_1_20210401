...
{
	struct nf_conn *ct_candidate = NULL, *ct_oldest_hwaccel = NULL;
	...
	if (!list_empty(&lo_safe_list)) {
		list_for_each_safe(tmp, next, &lo_safe_list) {
			struct nf_conn *ct = container_of(tmp, struct nf_conn, safe_list);
			if (unlikely(ct == NULL))
				continue;
			if (unlikely(ct == ct_oldest_hwaccel))
				break; /* already checked this connection, stop iteration here */
			...
			if (likely(nf_ct_is_confirmed(ct) && !nf_ct_is_dying(ct) &&
			    atomic_inc_not_zero(&ct->ct_general.use))) {
				if (likely(!ct->hw_accel_flows)) {
					ct_candidate = ct;
					if (ct_oldest_hwaccel != NULL)
						nf_ct_put(ct_oldest_hwaccel);
					break;
				} else {
					/* Move HW accelerated connections to the end of the list, so next time a connection has
					 * to be recycled this conntrack will have a small chance of being evaluated again */
					list_move_tail(&ct->safe_list, &lo_safe_list);
					if (unlikely(ct_oldest_hwaccel == NULL)){
						/* Recycle HW accelerated connections only when there is no other alternative */
						ct_oldest_hwaccel = ct;
					} else
						nf_ct_put(ct);
				}
			}
		}
		if (ct_candidate != NULL)
			list_move_tail(&ct_candidate->safe_list, &lo_safe_list);
		else if (ct_oldest_hwaccel != NULL)
			ct_candidate = ct_oldest_hwaccel;
	}

	if (ct_candidate == NULL && !list_empty(&hi_safe_list)) {
		list_for_each_safe(tmp, next, &hi_safe_list) {
			struct nf_conn *ct = container_of(tmp, struct nf_conn, safe_list);
			if (unlikely(ct == NULL))
				continue;
			if (unlikely(ct == ct_oldest_hwaccel))
		...
		if (ct_candidate != NULL)
			list_move_tail(&ct_candidate->safe_list, &hi_safe_list);
		else if (ct_oldest_hwaccel != NULL)
			ct_candidate = ct_oldest_hwaccel;
	}
...
#if defined(CONFIG_BCM_KF_BLOG) && defined(CONFIG_BCM_KF_NETFILTER) && defined(CONFIG_BLOG)
void ct_blog_flow_activate_event(void *ct_p, BlogFlowEventInfo_t info, BlogFlowEventType_t type)
{
	struct nf_conn *ct = ct_p;

	if (type == FLOW_EVENT_TYPE_HW)
		ct->hw_accel_flows++;
...
void ct_blog_flow_deactivate_event(void *ct_p, BlogFlowEventInfo_t info, BlogFlowEventType_t type)
{
	struct nf_conn *ct = ct_p;
		...
		struct sk_buff *skb)
{
	...
	if (ctinfo == IP_CT_ESTABLISHED ||
		...
		if (ct->iq_prio == IQOS_PRIO_HIGH)
			list_move_tail(&ct->safe_list, &hi_safe_list);
#if defined(CONFIG_BCM_KF_BLOG) && defined(CONFIG_BLOG)
		else if (new_reply && skb != NULL && skb->blog_p != NULL &&
				blog_iq(skb) == IQOS_PRIO_HIGH) {
			atomic_dec(&net->ct.iqloprio);
			list_del(&ct->safe_list);
			ct->iq_prio = IQOS_PRIO_HIGH;
			list_add_tail(&ct->safe_list, &hi_safe_list);
...
{
#if !(defined(CONFIG_BCM_KF_DPI) && defined(CONFIG_BRCM_DPI))
	/* DPI use these events, which for the time being
	 * do not support handlers chaining */
#if defined(CONFIG_BCM_KF_BLOG) && defined(CONFIG_BCM_KF_NETFILTER) && defined(CONFIG_BLOG)
	{
		BlogBind_t bind = { .bmap = { .FA_HOOK = 1, .FD_HOOK = 1 }, .hook_info = 0xFF };

		blog_bind_flow_event( NULL, NULL, bind);
	}
#endif
#endif
...
void nf_conntrack_init_end(void)
{
...
#if defined(CONFIG_BCM_KF_BLOG) && defined(CONFIG_BCM_KF_NETFILTER) && defined(CONFIG_BLOG)
	{
		...
		blog_bind_flow_event( ct_blog_flow_activate_event, ct_blog_flow_deactivate_event, bind);
	}
#endif
#else
	(void)ct_blog_flow_activate_event;
	(void)ct_blog_flow_deactivate_event;
#endif
