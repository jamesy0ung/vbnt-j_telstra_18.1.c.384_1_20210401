...
static int nand_tl_write_bbt(loff_t to, size_t len, size_t *retlen, const uint8_t *buf, int allow_bbt, int atomic);
static int nand_tl_write_blocklimited(loff_t to, struct mtd_oob_ops *ops, int isbbt, int atomic);
...
static int nand_tl_write(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen, const uint8_t *buf)
{
	...
	ret = nand_tl_write_bbt(to, len, retlen, buf, 0, OP_NO_ATOMIC);
	nand_tl_release_device();
	return ret;
}
...
static int nand_tl_write_bbt(loff_t to, size_t len, size_t *retlen, const uint8_t *buf, int allow_bbt, int atomic)
{
	...
	*retlen = 0;
	...
	do
	{
		...
		ret = nand_tl_write_blocklimited(nand_tl_trans_addr(to), &ops, allow_bbt, atomic);
...
static int nand_tl_write_blocklimited(loff_t to, struct mtd_oob_ops *ops, int isbbt, int atomic)
{
	...
	if (ops->len && ((to & (info_tl->mtd_tl->writesize - 1)) || ((to + ops->len) & (info_tl->mtd_tl->writesize - 1)) || atomic))
	{
		NAND_CODE_COV(NAND_CODE_COV_WRITE_UNALIN);
		NAND_DEBUG("nand write: not aligned OR atomic write enabled, using copyblocks.");
		if (isbbt)
			goto bbt_err_out;
...
static int nand_tl_copy_block(loff_t src, struct mtd_oob_ops *ops, loff_t dest)
{
	...
	ret = nand_tl_write_blocklimited(dest, &ops_tmp, 0, OP_NO_ATOMIC);
...
static int nand_tl_write_oob(struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops)
{
...

	{
		...
		do
		{
			ret = nand_tl_write_blocklimited(nand_tl_trans_addr(to), &ops_tmp, 0, OP_NO_ATOMIC);
			ops->oobretlen += ops_tmp.oobretlen;
...
static int nand_tl_bbt_save(void)
{
	...
	retlen = 0;
	ret = nand_tl_write_bbt(addr, bbt_size, &retlen, (uint8_t *)info_tl->tbbt, 1, OP_NO_ATOMIC);
...
static int nand_tl_proc_show(struct seq_file *file, void *v)
{
	nand_tl_bbt_print();
	return 0;
}

static int nand_tl_proc_fops_open(struct inode *inode, struct file *file)
{
	return single_open(file, nand_tl_proc_show, NULL);
}

static struct file_operations nand_tl_proc_fops = {
	.owner   = THIS_MODULE,
	.open    = nand_tl_proc_fops_open,
	.read    = seq_read,
...
static struct mtd_info *nand_tl_probe(struct map_info *map)
{
	...
	*/
	...
	proc_entry = proc_create("tbbt" ,0, NULL, &nand_tl_proc_fops);
	if (!proc_entry)
...
int mtd_tch_atomic_write(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen, const u_char *buf)
{
	...
	*retlen = 0;
	if (to < 0 || to >= mtd->size || len > mtd->size - to || (to & (info_tl->mtd_tl->erasesize - 1)) || (len != info_tl->mtd_tl->erasesize))
		return -EINVAL;
	...
	nand_tl_get_device(FL_WRITING);
	NAND_DEBUG("mtd: atomic writing to 0x%08x, len %d", (unsigned int)to, (unsigned int)len);
	ret = nand_tl_write_bbt(to, len, retlen, buf, 0, OP_ATOMIC);
	nand_tl_release_device();

	return ret;
}
EXPORT_SYMBOL_GPL(mtd_tch_atomic_write);
