...
static void prom_init_cmdline(int argc, char **argv)
{
	char *cp;
	int actr;
	...
	*cp++ = ' ';
	...
	while(actr < argc) {
        if (argv[actr][0] == '.') {
            prom_init_cmdline_handlehidden(&(argv[actr][1]));
        }
        else {
		strcpy(cp, argv[actr]);
		cp += strlen(argv[actr]);
...
void __init prom_init(void)
{
    ...
    if (boot_param_status_legacy == BOOT_FDT || early_init_dt_verify((void*)__dtb_start)) {
	...
	retrieve_boot_loader_parameters(argv+1);
        ...
        prom_init_cmdline(argc, (char **) fw_arg1);
#endif
    } else { 
