...
{
	unsigned long 		memsize = getBlMemorySize();
	...
	size = CONFIG_MASTER_POINTER_RESERVED_MEM;
	memsize -= CONFIG_MASTER_POINTER_RESERVED_MEM;
	reserve_system_mem(TCH_RESERVED_BASE_ADDR_STR, memsize, size);

	/* for GPON platform, need to reserve more memory due to RDPA */
#if defined(CONFIG_TECHNICOLOR_GPON_PATCH)
	size = memsize - CONFIG_GPON_OS_OFFSET;
	memsize = CONFIG_GPON_OS_OFFSET;
#else
	size = memsize - CONFIG_ADSL_OS_OFFSET;
	memsize = CONFIG_ADSL_OS_OFFSET;
#endif
	add_memory_region(memsize, size, BOOT_MEM_RAM);
...
static int __init bcm63268_hw_init(void)
{
    ...
    /* The DSL core has to be powered up at all times, as the secure boot rom
       implementation on the 63169 uses the memory of the DSL core. If the
       core would be powered down, secure boot would fail when doing a soft
       reset and the board would hang during reboot.
       See CS1949363 ([GPON-6483], BCM-CSP 835284)
    */
    if (1) {
#else
    if (0) {
...
#if defined(CONFIG_TECHNICOLOR_GPON_PATCH) && defined(CONFIG_BCM96838)
        /* The TCH GPON BCM96838 bootloader subtracts the LILAC TM and MC reserved mem; add it here again */
        mem_size += TM_DEF_DDR_SIZE + TM_MC_DEF_DDR_SIZE;
#endif
