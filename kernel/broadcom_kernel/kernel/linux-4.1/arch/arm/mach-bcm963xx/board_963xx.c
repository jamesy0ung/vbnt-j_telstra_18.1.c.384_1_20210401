...
#if !defined (CONFIG_OF) || defined(CONFIG_BCM_TCH_BL)
...
extern void * ProzoneMemory_phys;
...
static void __init board_fixup(struct tag *t, char **cmdline)
{
	...
	if (BpSetBoardId(tch_board) != BP_SUCCESS ||
	    BpGetCustomATAG_tch((void *)&t) != BP_SUCCESS) {
		/* if no custom ATAG table, use bootloader passed one */
		t = table_start;
	}
#if IS_ENABLED(CONFIG_BCM_DHD_RUNNER)
	dhd_pool_size[0] = 0;
...
static void __init board_reserve_atag(void)
{
	...
	mem_end -= MB_ALIGNED(TCH_RESERVED_TOPMEM);

	reserve_system_mem(TCH_RESERVED_BASE_ADDR_STR, mem_end, MB_ALIGNED(TCH_RESERVED_TOPMEM));

	/* get mem_end back 2M aligned for the DSL; can be improved */
	mem_end -= SZ_1M;
