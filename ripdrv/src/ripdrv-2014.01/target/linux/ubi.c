...
int rip2_flash_init(void *base, size_t size)
{
	...
	device->buffer = (unsigned char *) kmalloc(rip2_get_size(), GFP_KERNEL);
...
{
	if( device ) {
		return device->total_volume_size;
	}
	return -1;
}
...
{
	size_t retlen = 0;
	int rip2_size = rip2_get_size();
	unsigned char *read_buffer = device->buffer;

	memset(read_buffer, 0, rip2_size);
	return rip2_flash_read(RIP2_OFFSET, rip2_size, &retlen, read_buffer);
}
...
int rip2_flash_write(loff_t to, size_t len, size_t *retlen, unsigned char *buf)
{
	...
	if ((offs + len) > rip2_size)
		return -EINVAL;

	memcpy(read_buffer+offs, buf, len);

	bytes_left = rip2_size;
	while (bytes_left) {
