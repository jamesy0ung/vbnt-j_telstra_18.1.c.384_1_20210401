...
static int isSignatureValid(unsigned char* pSignature, const char* serialNumber, EFU_CHIPID_TYPE chipId,
    ...
    if(EFU_getOSIK(pubkey, pubkey_length) != EFU_RET_SUCCESS) {
        efu_dbg_printf("Unable to get OSIK\r\n");
...
unsigned int EFU_getOSIK(unsigned char * pubkey, unsigned long pubkey_length) {
    int ret;
    unsigned long key_len = pubkey_length;

    ret = rip2_drv_read(&pubkey_length, RIP_ID_OSIK, pubkey);

    if (ret != RIP2_SUCCESS)
    {
        if( sizeof(fallback_key) == key_len  ){
           memcpy(pubkey, fallback_key, key_len);
        } else {
           return EFU_RET_RIPERR;
        }
    }
    return EFU_RET_SUCCESS;
}
