...
static const struct blobmsg_policy iface_attrs[IFACE_ATTR_MAX] = {
	...
	[IFACE_ATTR_DNSSET] = { .name = "dnsset", .type = BLOBMSG_TYPE_STRING },
...
interface_alloc(const char *name, struct blob_attr *config)
{
	...
	if ((cur = tb[IFACE_ATTR_DNSSET])) {
		const char *dnsset = blobmsg_data(cur);
		if (strlen(dnsset) < sizeof(iface->proto_ip.dnsset)) {
			strcpy(iface->proto_ip.dnsset, dnsset);
			strcpy(iface->config_ip.dnsset, dnsset);
		} else
			netifd_log_message(L_WARNING, "DNSSET name too large for '%s'\n", iface->name);
	}
...
#define UPDATE(field, __var) ({						\
	...
	strcpy(if_old->proto_ip.dnsset, if_new->proto_ip.dnsset);
	strcpy(if_old->config_ip.dnsset, if_new->config_ip.dnsset);
