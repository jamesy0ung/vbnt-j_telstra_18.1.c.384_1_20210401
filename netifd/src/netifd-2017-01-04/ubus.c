...
void
netifd_ubus_ip_event(struct interface_ip_settings *ip, const int v6)
{
	void *a;

	blob_buf_init(&b, 0);
	a = blobmsg_open_array(&b, v6 ? "ipv6-address" : "ipv4-address");
	interface_ip_dump_address_list(ip, v6, true);
	blobmsg_close_array(&b, a);

	blobmsg_add_string(&b, "interface", ip->iface->name);
	ubus_send_event(ubus_ctx, "network.interface", b.head);
}
